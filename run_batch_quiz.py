...
def main():
    ...
    for model_idx, model in enumerate(models):
        ...
        for run in range(args.n):
            ...
            run_commitment = None
            try:
                run_commitment = quiz_llm.ask_self_id(model, api_key, systems, verbose=False) # verbose=False to keep output clean
                if run_commitment:
            ...
            try:
                ...
                scores = quiz_llm.run_quiz(model, api_key, dimensions, systems, verbose=False)
                
                if scores:
                    successful_runs += 1
                    top_match = scores[0]
                    print(f" Done. Top match: {top_match['name']} ({top_match['percentage']}%), Stated commitment: {run_commitment}")
